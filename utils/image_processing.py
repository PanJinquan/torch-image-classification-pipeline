...
def extend_rects(rects, scale=[0.5, 0.5]):
    """
    :param rects:
    :param scale: [sx,sy]
    :return:
...
def extend_bboxes(bboxes, scale=[0.5, 0.5]):
    """
...
class CVVideo():
    ...
    def start_capture(self, video_path, save_video=None, detect_freq=1):
        ...
        width, height, numFrames, fps = self.get_video_info(video_cap)
        if save_video:
    ...
    @staticmethod
    def get_get_video_capture(video_path, width=1280, height=1024):
        """
...
if __name__ == "__main__":
    ...
    # bboxes = [[100, 50, 280, 240]]
    # rects = bboxes2rects(bboxes)
    rects = [[100, 50, 180, 50]]
    print(rects)
    rect_image = show_image_rects("rect", image, rects, waitKey=1)
    rects2 = get_square_rects(rects, fixed="w")
